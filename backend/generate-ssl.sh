...
openssl req -new -x509 -key nginx/ssl/lifehub.key -out nginx/ssl/lifehub.crt -days 365 -subj "/C=FR/ST=IDF/L=Paris/O=LifeHub/OU=Development/CN=localhost/emailAddress=dev@lifehub.local" -addext "subjectAltName=DNS:localhost,DNS:lifehub.local,IP:127.0.0.1"
